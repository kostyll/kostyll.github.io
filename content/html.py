...
"""
Source gist : https://gist.github.com/3360002.git
Author      : kstep
Contributor : kostyll - https://kostyll.github.io
Date        : 2014/08/28

The main goal is to reach using this module in multi-threaded enveronment.
I've mode a small contribution - every context data is storing in a threads dict,
which key-field value is the name of the current thread - BE CAREFULL!!!
If there are two or more threads with the same name in runtime - you'll have logical
errors!
"""
...
class Context(object):
    ...
    @property
    def level(self):
        self.threads.setdefault(get_thread_identifier(),dict(level=0))
        return self.threads[get_thread_identifier()]['level']

    @level.setter
    def level(self,value):
        self.threads.setdefault(get_thread_identifier(),dict(level=0))
        self.threads[get_thread_identifier()]['level'] = value
    ...
    def __enter__(self):
        if threading is None:
            ...
            self.threads.setdefault(get_thread_identifier(),{'level':0})
            self.threads[get_thread_identifier()]['level'] += 1
    ...
    #     else:
    #         print(self.indent * self.threads[get_thread_identifier()]['level'] + str(data))
    #     return self
...
class StrContext(Context):
    ...
    def __lshift__(self, data):
        if threading is None:
            ...
            self.threads.setdefault(get_thread_identifier(),{'level':0})
            self.threads[get_thread_identifier()].setdefault('data','')
            self.threads[get_thread_identifier()]['data'] += self.indent * self.threads[get_thread_identifier()]['level'] + str(data) + "\n"
        return self
    ...
    def __str__(self):
        if threading is None:
            ...
            return self.threads[get_thread_identifier()]['data']
